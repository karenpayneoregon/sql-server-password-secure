...
*/
DECLARE @IncludeModifiers BIT
        = 1,                      /* set to 1 to add permutations of the main password list, replacing 'a' with '@', '3' with '#', 's' with '$', 'o' with '*' */
...
IF OBJECT_ID('tempdb..#pwd') IS NOT NULL
    DROP TABLE #pwd;
CREATE TABLE #pwd
(
...
(
    VALUES
        ...
        (N'йцукенгшщзхъ'),
        (N'какашка'),
        (N'концертных площадок и умных студентов:'),
        (N'кристина'),
        (N'любимая'),
        (N'любимый'),
        (N'люблю'),
...
;WITH NumbersCTE
AS (SELECT CONVERT(NVARCHAR(MAX), '') AS generatedPwd,
           0 AS currLength,
           Numbers.num AS Pwdmaxlength,
           V.TxtSkip,
           L.SkipLocation
    FROM
    (
    ...
    UNION ALL
    SELECT CTE.generatedPwd + CASE
                                  WHEN CTE.SkipLocation = 'before' THEN
                                      ISNULL(CTE.TxtSkip, CONVERT(NVARCHAR(MAX), CTE.currLength + 1))
                                  ELSE
                                      ''
                              END + CONVERT(NVARCHAR(MAX), CTE.currLength + 1)
           + CASE
                 WHEN CTE.SkipLocation = 'after' THEN
                     ISNULL(CTE.TxtSkip, CONVERT(NVARCHAR(MAX), CTE.currLength + 1))
                 ELSE
                     ''
             END,
...
AS (
   ...
   (
       ...
       (
           SELECT TOP (12)
                  ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS Occurences
           FROM sys.all_columns
       ) AS Occ
           ON Occ.Occurences
              BETWEEN V.MinOccurences AND V.MaxOccurences
   UNION ALL

   -- Common permutations (singles)
   SELECT CommonPermutationWords.TxtWord
...
-- Generated passwords
INSERT INTO #pwd WITH (TABLOCKX)
SELECT LTRIM(RTRIM(GeneratedPasswords.generatedPwd))
...
BEGIN
    ...
    FROM #pwd
    WHERE LEN(generatedPwd) > 2
          AND generatedPwd LIKE '[a-z]%'
    UNION ALL
...
SELECT Deviation = dev.Deviation + CASE
                                       WHEN @OutputPasswords = 1 THEN
                                           N' (' + dev.Pwd + N')'
                                       ELSE
                                           N''
                                   END,
...
FROM
(
    SELECT 'Empty Password' AS Deviation,
           s.sid,
           s.principal_id,
           RTRIM(s.name) AS LoginName,
           '' AS Pwd
    ...
    WHERE s.is_disabled = 0
          ...
          (
              ...
              OR PWDCOMPARE('', s.password_hash) = 1
          )
          AND s.name NOT IN ( 'MSCRMSqlClrLogin' )
          AND s.name NOT LIKE '##MS[_]%##'
    UNION ALL
    SELECT DISTINCT
    ...
    UNION ALL
    SELECT 'Weak Password' AS Deviation,
           s.sid,
           s.principal_id,
           RTRIM(s.name) AS LoginName,
           d.generatedPwd
    ...
    (SELECT DISTINCT generatedPwd FROM #pwd) AS d
        INNER JOIN sys.sql_logins s
            ON PWDCOMPARE(d.generatedPwd, s.password_hash) = 1
    WHERE s.is_disabled = 0
) AS dev
OPTION (RECOMPILE); -- avoid saving this in plan cache
...
BEGIN
    UPDATE dev
    SET dev.ServerPermissions = STUFF(
                                (
                                    SELECT N', ' + perm.state_desc + N' ' + perm.permission_name + N' '
                                           + perm.class_desc
...
FROM #logins
    ...
    (
        ...
        ON activesessions.login_name COLLATE DATABASE_DEFAULT = LoginName COLLATE DATABASE_DEFAULT
OPTION (RECOMPILE); -- avoid saving this in plan cache

SELECT COUNT(DISTINCT generatedPwd) AS DistinctPasswords,
       COUNT(*) AS TotalPasswords
FROM #pwd;
